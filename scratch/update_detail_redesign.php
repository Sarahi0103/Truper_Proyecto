<?php
$filePath = __DIR__ . '/../public/update_detail.php';
$content = file_get_contents($filePath);

if (!$content) {
    die("Error: Could not read update_detail.php\n");
}

echo "Updating CSS Style tag...\n";

// Replace style block
$startStyle = strpos($content, '<style>');
$endStyle = strpos($content, '</style>', $startStyle);

if ($startStyle === false || $endStyle === false) {
    die("Error: Could not find <style> or </style> in update_detail.php\n");
}
...
$content = substr_replace($content, $newCss, $startStyle, ($endStyle - $startStyle) + 8);

echo "Updating HTML Template 2: SPLIT...\n";

// Replace tpl-split block
$splitSearch = '        <?php if ($tpl === \'split\'): ?>
...
// Normalise line endings to avoid failure
$content = str_replace(
    array("\r\n", "\r"),
    "\n",
    $content
);
$splitSearch = str_replace(
    array("\r\n", "\r"),
    "\n",
    $splitSearch
);
$splitReplace = str_replace(
    array("\r\n", "\r"),
    "\n",
    $splitReplace
);

if (strpos($content, $splitSearch) === false) {
    echo "Warning: Could not find exact Split Search block. Trying dynamic replace...\n";
    // We'll fall back to direct search for key components if exact match fails
} else {
    $content = str_replace($splitSearch, $splitReplace, $content);
    echo "✓ Split Layout updated.\n";
}

echo "Updating HTML Template 3: GALLERY...\n";

$gallerySearch = '        <?php elseif ($tpl === \'gallery\'): ?>
...
$galleryReplace = '        <?php elseif ($tpl === \'gallery\'): ?>
            <!-- TEMPLATE 3: GALLERY FOCUS LAYOUT -->
...
$gallerySearch = str_replace(array("\r\n", "\r"), "\n", $gallerySearch);
$galleryReplace = str_replace(array("\r\n", "\r"), "\n", $galleryReplace);

if (strpos($content, $gallerySearch) === false) {
    echo "Warning: Could not find exact Gallery Search block.\n";
} else {
    $content = str_replace($gallerySearch, $galleryReplace, $content);
    echo "✓ Gallery Layout updated.\n";
}

echo "Updating HTML Template 4: MINIMAL...\n";

$minimalSearch = '        <?php elseif ($tpl === \'minimal\'): ?>
...
$minimalReplace = '        <?php elseif ($tpl === \'minimal\'): ?>
            <!-- TEMPLATE 4: MINIMAL PREMIUM LAYOUT -->
...
$minimalSearch = str_replace(array("\r\n", "\r"), "\n", $minimalSearch);
$minimalReplace = str_replace(array("\r\n", "\r"), "\n", $minimalReplace);

if (strpos($content, $minimalSearch) === false) {
    echo "Warning: Could not find exact Minimal Search block.\n";
} else {
    $content = str_replace($minimalSearch, $minimalReplace, $content);
    echo "✓ Minimal Layout updated.\n";
}

echo "Updating HTML Template 5: MAGAZINE...\n";

$magazineSearch = '        <?php elseif ($tpl === \'magazine\'): ?>
...
$magazineReplace = '        <?php elseif ($tpl === \'magazine\'): ?>
            <!-- TEMPLATE 5: MAGAZINE LAYOUT -->
...
$magazineSearch = str_replace(array("\r\n", "\r"), "\n", $magazineSearch);
$magazineReplace = str_replace(array("\r\n", "\r"), "\n", $magazineReplace);

if (strpos($content, $magazineSearch) === false) {
    echo "Warning: Could not find exact Magazine Search block.\n";
} else {
    $content = str_replace($magazineSearch, $magazineReplace, $content);
    echo "✓ Magazine Layout updated.\n";
}

echo "Updating HTML Template 1: CLASSIC...\n";

$classicSearch = '        <?php else: ?>
...
$classicSearch = str_replace(array("\r\n", "\r"), "\n", $classicSearch);
$classicReplace = str_replace(array("\r\n", "\r"), "\n", $classicReplace);

if (strpos($content, $classicSearch) === false) {
    echo "Warning: Could not find exact Classic Search block.\n";
} else {
    $content = str_replace($classicSearch, $classicReplace, $content);
    echo "✓ Classic Layout updated.\n";
}

// Save back
file_put_contents($filePath, $content);
echo "All replacements applied successfully! Running syntax validation...\n";
...
if ($return_var === 0) {
    echo "✓ PHP Syntax validation SUCCESS.\n";
} else {
